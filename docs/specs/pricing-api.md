...
> This API publishes information about Autonomi: token supply and dated upload-cost references. It is not a hosted gateway for uploading or retrieving network data. For network access, run a supported client locally: antd exposes local REST and gRPC interfaces, daemon-backed SDKs and MCP tools call antd, while the ant CLI and Rust ant-core client can access the network directly. Choose an interface below and follow its setup guide; installing a daemon does not start its service.
